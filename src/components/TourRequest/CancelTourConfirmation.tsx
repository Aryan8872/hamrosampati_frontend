...
const CancelTourConfirmation = ({
  onOpen,
  onClose,
  dataId,
  title = "Cancel Tour Booking",
  message = "Are you sure you want to cancel the Tour booking? This action cannot be undone.",
  confirmText = "Confirm cancel",
  cancelText = "Cancel",
}: ConfirmationModalProps) => {
  const { useCancelTourBooking } = useTour();
  const { mutateAsync, isPending } = useCancelTourBooking();

  const confirmCancel = async () => {
    try {
      await mutateAsync(dataId!);
      onClose();
...
export default CancelTourConfirmation;